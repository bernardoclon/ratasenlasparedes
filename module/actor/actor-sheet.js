        ...
        roll: {
          ...
          callback: async (dlg) => {
            ...
            await ChatMessage.create({
              ...
              type: foundry.CONST.CHAT_MESSAGE_STYLES.ROLL,
              content: diceHtml,
              roll: result
            });
...
Hooks.on('renderChatMessage', (message, html, data) => {
    html.find('.rollable[data-roll-type="spell-sanity"]').click(async ev => {
        ...
        if (actor && typeof actor.system.pc?.value === 'number') {
            const currentSanity = actor.system.pc.value;
            const newSanity = currentSanity - sanityLoss;
            await actor.update({'system.pc.value': newSanity});
        }
