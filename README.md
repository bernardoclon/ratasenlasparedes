![banner8](https://github.com/user-attachments/assets/39be5165-c457-4a9a-84a2-41e48aa13899)

# 📜 Ratas en las Paredes: El Sistema Digital

Hemos trasvasado la tinta maldita al código binario. Esta selección de herramientas impías ha sido diseñada para narrar el inexorable descenso hacia la locura. Cada clic en esta interfaz es un rasguño más en la puerta que retiene a los horrores exteriores.

## 🗝️ Las Llaves de la Locura (Características)

Lo que yace dormido ha despertado con nuevas formas. El sistema ahora respira por sí mismo:

### ⚙️ Automatización de los Designios
Ya no necesitas calcular tu condena manualmente. **La automatización de reglas** se encarga de gestionar los éxitos, los fracasos y las consecuencias. El sistema interpreta los ritos mecánicos para que tú solo tengas que preocuparte por narrar el horror resultante.

### 🩸 La Contabilidad de la Carne y la Mente
Monitoriza con precisión clínica la fragilidad humana. El sistema gestiona automáticamente los **Puntos de Cordura y de Salud**. Observa cómo la barra de vida desciende hacia la tumba y cómo la psique de tus investigadores se desmorona ante la revelación de verdades cósmicas. Un contador regresivo hacia el manicomio... o algo peor.

### 📚 Bibliotecas de lo Oculto (Compendios)
No llegas con las manos vacías ante el vacío. Hemos incluido **compendios repletos de lo necesario para tejer tus misterios**. Objetos malditos, horrores primigenios y fragmentos de conocimiento prohibido listos para ser arrastrados a tu mesa de juego y atormentar a tus jugadores.

### 🕯️ Inmersión en el Abismo
La realidad se disuelve frente a tus ojos. Hemos diseñado una **estética de hojas de personaje y cuadros de diálogo** que transforman la pantalla en un legajo antiguo y perturbador. La experiencia visual es ahora totalmente inmersiva, haciendo que cada ficha parezca un documento rescatado de una casa en ruinas.

---

> **👁️ Nota del Guardián:**
> Para que la experiencia sea completa y el sonido del destino al caer tenga peso, **no olvides instalar el módulo "Dice So Nice"**. Es la ofrenda final para que los dados rueden como huesos secos sobre la mesa virtual.

---

## 📦 El Ritual de Invocación (Instalación)

¿Te atreves a abrir la puerta? Si deseas invocar esta aberración en tu mesa virtual, sigue los pasos del ritual:

1. Abre **Foundry VTT** y contempla el vacío de tu configuración.
2. Dirígete a la pestaña **Systems**.
3. Haz clic en **Install System**.
4. Pega el siguiente conjuro en el campo **Manifest URL**:
```   
https://raw.githubusercontent.com/bernardoclon/ratasenlasparedes/refs/heads/main/system.json
```
6. Haz clic en **Install**. Reza a los dioses arquetípicos para que tu cordura resista lo que has desatado.

## ⚠️ Advertencia de los Antiguos

*"No está muerto lo que puede yacer eternamente, y con los evos extraños, incluso la muerte puede morir."*

Este sistema es una herramienta para narrar el horror cósmico. No busca el equilibrio, ni la victoria heroica, sino la simulación de la fragilidad humana ante fuerzas incomprensibles. No nos hacemos responsables de las pesadillas recurrentes o de la repentina aversión a las esquinas oscuras de tu habitación.

## 🏛️ Reconocimientos a los Artífices

Nuestra gratitud, teñida de temor reverencial, se dirige a los artífices primigenios de esta genialidad palpable en el yermo digital: **jvir, NeV3rKilL y ParvusDomus**. Sin su obra, el caos no podría haber sido ordenado en código.

A **The Hill Press**, por poner el tomo original en nuestras manos temblorosas y permitir el uso de su iconografía sagrada.

* Los iconos utilizados son ofrendas de **game-icons.net**.
* Los derechos de la obra original *"Ratas en las Paredes"* pertenecen a **The Hill Press**.
* Cualquier derecho sobre la arquitectura digital de este sistema, cuyo autor principal es **jvir**, queda bajo la licencia **CC-BY-SA**.

*Ph'nglui mglw'nafh Cthulhu R'lyeh wgah'nagl fhtagn.*
